...
const NewAppButton: FC<Props> = ({ size = 60 }) => {
  ...
  const presetIds = useMemo(() => new Set(ORIGIN_DEFAULT_MIN_APPS.map((app) => app.id)), [])
  ...
  const validateForm = () => {
    ...
    if (normalizedId) {
      ...
      if (idError) {
        nextErrors.id = t(`settings.miniapps.custom.${idError}`, { ids: normalizedId })
      }
    }
          ...
          <form className="flex min-h-0 flex-1 flex-col" onSubmit={handleSubmit}>
            <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto px-5 py-5">
              ...
              <div className="space-y-3">
                <div className="text-sm font-medium text-[var(--color-text-1)]">{t('settings.miniapps.custom.logo')}</div>
                ...
                {logoType === 'url' ? (
                  ...
                  <div className="space-y-2">
                    <div className="text-sm text-[var(--color-text-2)]">{t('settings.miniapps.custom.logo_upload_label')}</div>
                    <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleFileChange} />
                    <Button type="button" variant="outline" className="w-full justify-center gap-2" onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="rounded-2xl border border-dashed border-[var(--color-border)] bg-[var(--color-background-soft)] p-4">
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    <div className="flex size-14 items-center justify-center overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)]">
                      {hasLogoPreview ? (
                        <img src={formValues.logo} alt={formValues.name || t('settings.miniapps.custom.title')} className="h-full w-full object-cover" />
                      ) : (
