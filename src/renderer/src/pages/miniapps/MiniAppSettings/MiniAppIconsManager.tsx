...
const MiniAppIconsManager: FC<MiniAppManagerProps> = ({
  ...
  const onMoveMiniApp = useCallback(
    (app: MiniApp, fromList: ListType) => {
      ...
      const newVisible = isMovingToVisible ? [...visibleMiniApps, app] : visibleMiniApps.filter((item) => item.appId !== app.appId)
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="grid gap-4 lg:grid-cols-2">
        {(['visible', 'disabled'] as const).map((listType) => (
          <div key={listType} className="min-w-0 rounded-[24px] border border-[var(--color-border)] bg-[var(--color-background-soft)]/70 p-3">
